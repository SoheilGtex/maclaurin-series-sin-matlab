...
**RESULTS PENDING MATLAB EXECUTION.** This repository does not contain fabricated numerical values. Running `addpath('src'); addpath('experiments'); generateAllFigures;` creates raw CSV files, selected summaries, and figures under `results/`. The environment used to prepare this archive did not contain MATLAB or GNU Octave, so no empirical claim is made here.

When genuine results are available, this section should include the MATLAB release, platform, selected tables from `results/reference`, plots from `results/figures`, and interpretation tied to observed—not assumed—behavior.
...
The repository isolates one function so that mathematical approximation theory and implementation effects can be examined together. Its conclusions must be updated from genuine generated evidence after MATLAB execution; the current archive intentionally separates implemented code from executed-and-verified results.
